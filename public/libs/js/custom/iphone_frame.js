$(function () {
  ...
  <div class="tablaContent"></div>
            ...
            globalState.dateTime.dias.map((dia) =>
                header.append(
                    `<div class="diaName">${
                        config.diaCompleto ? dia : dia.charAt(0)
                    }</div>`
                )
            );
        ...
        fechaIcono: function (config) {
            ...
            this.append(
                `<div class="fechaWrapper"><p class="diaNom">${
        ...
        reloj: function () {
            ...
            let transformHora = `calc(${
                (360 / 12 - 360) * tiempo.getHours()
            }deg + ${(30 / 60) * tiempo.getMinutes()}deg)`;
            let transformMinutos = `calc(6deg * ${tiempo.getMinutes()} + ${
                (6 / 60) * tiempo.getSeconds()
            }deg)`;
    ...
    function pintarApps(apps, container, containerDots) {
        ...
        apps.map((app, idArr) => {
            ...
            if (app.dinamico && app.type == "app")
                clases = `${clases} ${sanearString(
                    app.nombre
                ).toLowerCase()}Dinamico`;
            html += `<div class="${clases}" data-app="${
                app.type + sanearString(app.nombre)
            }" data-id="${idArr}">
  ${
      app.notificaciones
  ...
  <p class="nombre">${app.nombre}</p>
        ...
        if (globalState.wrapperApps.grupos > 1) {
            for (
                let index = 0;
                index < globalState.wrapperApps.grupos;
                index++
            ) {
  ...
  `);
        if (config.closable)
            $("#iOSAlert").prepend('<div class="closable"></div>');
        $("#iOSAlert")
  ...
  <path d="M62 38l-7.259-9.86L46 36"></path>
    ...
    function renderizarUI() {
        ...
        if ($('.wrapperApps .app[data-app="widgetFullCalendario"]').length) {
            ...
            $(
                '.wrapperApps .app[data-app="widgetFullCalendario"] .icono'
            ).append(
    ...
    $(".wrapperApps").touchMov({
        updateMovX: function (e, mov) {
            $(e.currentTarget).css({
                transform: `translateX(${
                    globalState.wrapperApps.transform + mov
                }px)`,
                transition: "none",
            });
        },
        movIzq: function (e) {
            if (
                ...
                globalState.wrapperApps.grupos
            ) {
                globalState.wrapperApps.grupoActivo++;
            }
            $(e.currentTarget).css({
        ...
        movDer: function (e) {
            if (globalState.wrapperApps.grupoActivo != 1) {
                ...
                $(e.currentTarget).css({
                    transform: `translateX(${
                        globalState.wrapperApps.transform +
                        globalState.wrapperApps.medida
                    }px)`,
                    transition: "ease all 0.2s",
                });
    ...
    $(".mainScreen .appScreen").mousedown(function (e) {
        ...
        let timeout = setTimeout(() => {
            ...
            if (!globalState.draggScreen) {
                if (
                    $(e.target).hasClass("app") ||
                    $(e.target).parents(".app").length
                ) {
  ...
  `);
                    ...
                    $(".appScreen .app").append(
                        '<div class="removeApp"></div>'
                    );
                }
            }
        }, 1000);
    ...
    $("body").on(
        "click",
        ".fixedMenuFixedApp .menuOption.shaking",
        function () {
            $(this).parent().remove();
            $("#fixedApp").remove();
    ...
    $(".widgetPlus").click(function () {
        ...
        $(".appScreen .app .removeApp").remove();
        $(".mainScreen")
            .removeClass("shakingApps")
            .addClass("widgetScreenOpen");
    });
    //Eliminar app
    $("body").on(
        "click",
        ".fixedMenuFixedApp .menuOption.eliminar",
        function () {
            ...
            alertaiOS({
                encabezado: `¿Transferir ${
                    idApp !== undefined ? globalState.apps[idApp].nombre : "app"
            ...
            acciones: [
                {
                    ...
                    callback: function () {
                        if (idApp !== undefined) {
                            ...
                            $(
                                '.deckApps .app[data-indeck="' + idDeck + '"]'
                            ).remove();
                        }
                    },
                },
